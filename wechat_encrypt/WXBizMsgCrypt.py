...
<Nonce><![CDATA[%(nonce)s]]></Nonce>
    ...
    def extract(self, xmltext, appid=False):
        """提取出xml数据包中的加密消息
...
class WXBizMsgCrypt(object):
    ...
    def EncryptMsg(self, sReplyMsg, sNonce, timestamp = None):
        ...
        logger.info(u'encrypt is: %s' % encrypt)
        if ret != 0:
            return ret,None
    ...
    def DecryptMsg(self, sPostData, sMsgSignature, sTimeStamp, sNonce, app=False):
        # 检验消息的真实性，并且获取解密后的明文
        ...
        # @return: 成功0，失败返回对应的错误码
         # 验证安全签名
        xmlParse = XMLParse()
        ret,encrypt,touser_name = xmlParse.extract(sPostData, app)
        if ret != 0:
            return ret, None
        sha1 = SHA1()
